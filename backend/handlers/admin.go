...
// CreateAdminRequest defines the JSON payload for onboarding a LibraryAdmin.
...
// CreateAdmin allows the library owner to onboard a new LibraryAdmin.
...
func ApproveIssueRequest(c *gin.Context) {
	...
	if err := tx.Where("isbn = ? AND lib_id = ?", reqEvent.BookID, libID).First(&book).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Database error checking book availability"})
		return
	}
	...
	now := time.Now()
	if err := tx.Model(&reqEvent).Updates(models.RequestEvent{ApprovalDate: &now, ApproverID: &user.ID}).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Database error updating request event"})
		return
	}
	...
	if err := tx.Create(&issue).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Database error inserting into issue registry"})
		return
	}
